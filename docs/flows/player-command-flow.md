...
27. `receiveMessage` parses JSON and then:
   ...
   - Applies correlated `cmd.*.success`, `cmd.*.error`, and asynchronous
     `cmd.*.cancelled` results to their `data.request_segment` immediately. The
     output and the echo's `✓` or red `×` therefore render from the same
     WebSocket message.
...
31. `Message.vue` renders plain text by splitting `message.text` on newline
    ...
    symbol on the same line: `…` while unresolved, `✓` after success, or a red
    `×` on failure. Its disclosure shows precise details on hover, keyboard
    focus, or mobile tap.
