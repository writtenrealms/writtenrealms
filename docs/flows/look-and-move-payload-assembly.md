...
3. `LookAction`:
   ...
   - resolves an exact direction or direction abbreviation to a room-facing
     door before ordinary character/item targeting; this is a single indexed,
     read-only query and does not materialize lazy `DoorState`
   - builds:
...
### Resulting payload shape (directional door look)
...
```json
{
  ...
  "text": "The bronze is closed."
}
```

The effective state is the runtime world's `DoorState` when present and the
doorway's authored default otherwise. Parallel runtime worlds remain isolated.
...
- `cmd.look.success` text uses the target payload appropriate to its
  `target_type`; directional doors render as `The <name> is <state>.`
...
- `cmd.look.success` (door) -> `Message.vue`
