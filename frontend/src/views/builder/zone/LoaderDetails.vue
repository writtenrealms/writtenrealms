...
const isNew = computed(() => String(route.params.loader_id) === "new");
...
const endpoint = computed(() => (
  `/builder/worlds/${route.params.world_id}/zones/${route.params.zone_id}/loaders/${route.params.loader_id}/`
));
const manifestApplyEndpoint = computed(() => `/builder/worlds/${route.params.world_id}/manifests/apply/`);
const headerTitle = computed(() => {
  if (spawnPlan.value?.name) return spawnPlan.value.name;
  return "New Spawn Plan";
});
...
const newSpawnPlanYaml = () => {
  ...
  const slug = slugifyName(`${zoneName} Spawn Plan`);
...
const syncRouteToSpawnPlan = async (payload: any) => {
  ...
  if (
    String(route.params.loader_id) === String(id)
...
watch(
  () => [route.params.zone_id, route.params.loader_id],
