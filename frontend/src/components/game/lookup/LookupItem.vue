...
const hasAction = (actionCode: string) => {
  const itemActions = item.value && item.value.actions;
...
const actions = computed(() => {
  ...
  const actionMap = (
    item.value &&
    item.value.actions &&
    !Array.isArray(item.value.actions)
  ) ? item.value.actions : {};
...
const doAction = (item, action) => {
  ...
  let payload: any = {};
  const itemKeyword = getKeyword(item);
  ...
  for (const room_item of store.state.game.room.inventory) {
    if (room_item.key === item.key) {
      ...
      const foundIndex = (room_item.keywords || "").split(" ").indexOf(itemKeyword);
      if (foundIndex !== -1) {
        duplicateCount += 1;
      }
    }
  ...
  if (!in_room) {
    ...
    for (const inv_item of store.state.game.player.inventory) {
      if (inv_item.key === item.key) {
        ...
        const foundIndex = (inv_item.keywords || "").split(" ").indexOf(itemKeyword);
        if (foundIndex !== -1) {
          duplicateCount += 1;
        }
      }
  ...
  let item_keyword = itemKeyword || item.name;
