...
class CurrencyPayloadTests(WorldTestCase):
    ...
    def test_extraction_cannot_overwrite_wallet_revision(self):
        self.player.wallet_revision = 7
        self.player.save(update_fields=["wallet_revision", "modified_ts"])
        serializer = ExtractPlayerSerializer(
            self.player,
            data={"wallet_revision": 999},
            partial=True,
        )
        ...
        self.assertEqual(self.player.wallet_revision, 7)
