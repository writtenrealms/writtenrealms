...
class TriggerManifestTests(BuilderTestCase):
    ...
    def test_room_trigger_list_includes_yaml_manifest(self):
        ...
        template_yaml = resp.data["new_trigger_template"]["yaml"]
        self.assertIn("kind: trigger", template_yaml)
        self.assertIn(f"world: world.{self.world.id}", template_yaml)
        self.assertIn(f"key: room.{self.room.id}", template_yaml)
