...
class TestCreateWorld(WorldTestCase):
    ...
    def test_create_instance(self):
        ...
        self.world.config.death_currency_penalty = 0.35
        ...
        self.assertEqual(float(instance.config.death_currency_penalty), 0.35)
        self.assertNotEqual(
...
class WorldFactionTests(BuilderTestCase):
    ...
    def test_add_core_faction(self):
        ...
        self.assertTrue(faction.playable)

    def test_add_reputation_faction(self):
        resp = self.client.post(self.factions_ep, {
            'code': 'myfaction',
            'name': 'My Faction',
            'type': 'reputation',
        ...
        self.assertEqual(Faction.objects.count(), 1)
        faction = Faction.objects.get()
        self.assertEqual(faction.type, 'reputation')
        self.assertFalse(faction.playable)

    def test_canonical_fields_override_legacy_aliases(self):
        resp = self.client.post(self.factions_ep, {
            'code': 'myfaction',
            'name': 'My Faction',
            'type': 'core',
        ...
        faction = Faction.objects.get()
        self.assertEqual(faction.type, 'core')
        self.assertTrue(faction.is_core)
        self.assertFalse(faction.playable)
        self.assertFalse(faction.is_selectable)
    ...
    def test_canonical_default_must_be_playable(self):
        faction = Faction.objects.create(
            code='myfaction',
            name='My Faction',
            world=self.world,
    ...
    def test_change_reputation_faction_to_core_with_conflict(self):
        """
        A player cannot acquire two core factions when a builder changes a
        reputation faction's type.
        """
...
class WorldFactionRankTests(BuilderTestCase):
    ...
    def setUp(self):
        ...
        self.faction_ranks_ep = reverse(
            ...
            args=[self.world.pk, self.faction.pk])
