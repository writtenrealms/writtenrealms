...
class TestRunWorldSpawnPlansTask(WorldTestCase):
    def setUp(self):
        super().setUp()
        self.spawn_world.set_lifecycle(api_consts.WORLD_LIFECYCLE_RUNNING)

    @mock.patch('worlds.tasks.run_spawn_plans_for_world')
    def test_runs_for_running_spawn_worlds_only(self, mock_run_spawn_plans):
        ...
        called_world_ids = {
            ...
            for call in mock_run_spawn_plans.call_args_list
        }
    ...
    @mock.patch('worlds.tasks.run_spawn_plans_for_world')
    def test_continues_after_individual_world_failure(self, mock_run_spawn_plans):
