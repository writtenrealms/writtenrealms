...
class TestStartWorld(WorldTestCase):
    ...
    def test_reset_world(self):
        ...
        spawn_room = spawn_world.context.config.starting_room
        self.player.world = spawn_world
        self.player.room = spawn_room
        ...
        self.player.equipment.weapon = equipped_item
        self.player.equipment.save(update_fields=['weapon'])
        mob_bag = Item.objects.create(
            world=spawn_world,
            container=mob,
            type=api_consts.ITEM_TYPE_CONTAINER,
            name="Bag",
        )
        nested_mob_item = Item.objects.create(
            world=spawn_world,
            container=mob_bag,
            name="Book",
        )
        ...
        self.assertFalse(Item.objects.filter(pk=mob_bag.pk).exists())
        self.assertFalse(Item.objects.filter(pk=nested_mob_item.pk).exists())
        self.assertTrue(Item.objects.filter(pk=held_item.pk).exists())
        self.assertTrue(Item.objects.filter(pk=equipped_item.pk).exists())
