...
class ForgotPasswordTests(WorldTestCase):
    ...
    def test_request_login_link(self, mock_send):
        # Passwordless login creates an unconfirmed account for a new email
        # while preserving the same public response as an existing account.
        resp = self.client.post(reverse('forgot-password'), {
            'email': 'nobody@example.com'
        })
        self.assertEqual(resp.status_code, 201)
        unknown_user = User.objects.get(email='nobody@example.com')
        self.assertFalse(unknown_user.is_confirmed)
        self.assertEqual(
            LoginLinkRequest.objects.filter(user=unknown_user).count(),
            1)
