...
## Current Trigger + Manifest Baseline

WR2 now has an implemented authored-trigger baseline that sits on top of the Command → Action → Event direction:

- Trigger authored model exists (`backend/builders/models.py`).
- Trigger command kind is `command`.
- Runtime command-trigger execution is wired (`backend/spawns/triggers.py`) and integrated into unknown text fallback (`backend/spawns/handlers/text.py`).
- Builder room UI shows **Triggers**.
- Builder world UI has **Edit World** manifest ingestion.
- Trigger YAML manifest create/update/delete flow is implemented in `backend/builders/manifests.py` and builder manifest endpoint handlers.

Trigger manifest capabilities currently implemented:

- create: omit `metadata.id` / `metadata.key`
- update: provide `metadata.id` or `metadata.key`
- delete: `operation: delete` + `metadata.id` or `metadata.key`

Usability rules currently implemented:

- `kind` is case-insensitive on ingest.
- `metadata.world` accepts either `1` or `world.1`.
- updates can be minimal patches (only changed fields in `spec`).
