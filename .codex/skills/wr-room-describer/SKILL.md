...
description: Draft or update Written Realms room descriptions from database room IDs, builder notes, exits, and nearby room context. Use when the user wants Codex to write, revise, populate, or review text-MUD room descriptions in the Edeus/Written Realms style using WR room IDs, Django database rooms, local room-ID text exports, notes, keywords, or exported room data.
---
...
4. Use `scripts/db_room_context.py context --room-id <id> --depth 3` to gather the target room, connected rooms, coordinate-adjacent rooms, and world context from the Django database. Always read the world context before drafting.
   ...
   - The helper reads `Room.name`, `Room.note`, `Room.description`, `Room.zone`, coordinates, and `north/east/south/west/up/down`, plus the world's name and description.
   - For an instance world, the helper also reads the base world's name and description. Use the instance description as the local lens and the base-world description as the broader setting anchor.
...
- Anchor the room in the supplied world description. For instance worlds, respect both descriptions, using the base world for broad setting continuity and the instance world for its more specific place, atmosphere, and scope.
